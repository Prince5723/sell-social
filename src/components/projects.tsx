      ...
      {cards.map((card, i) => (
        <div
          ...
          className={cn(
            ...
            "bg-white rounded-lg overflow-hidden shadow-md hover:shadow-xl transition-shadow duration-300 cursor-pointer relative h-64"
          )}
          onClick={() => handleCardClick(card.link)}
        >
          <div className="w-full h-full border boder-slate-50 rounded-lg overflow-hidden">
            <img
              src={card.thumbnail}
              alt={card.title}
              className="object-cover object-top h-full w-full transition-all duration-500 ease-in-out transform hover:scale-110"
            />
          </div>
